...
module.exports = {
  ...
  // Create event with linked user - copied from Strapi docs
  async create (ctx) {
    let entity;
    if (ctx.is('multipart')) {
  ...
  // Update user events - copied from Strapi docs
  async update(ctx) {
    const { id } = ctx.params;
  ...
  // Get logged in users - custom endpoint
  async me (ctx) {
    const user = ctx.state.user

    if (!user) {
      return ctx.badRequest(null, [{ messages: {id: 'No authorization header was fround'}}])
    }
